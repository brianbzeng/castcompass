...
| Browser maintenance document | Release/security operations; public and content-free | No cache key is permitted | Browser and CDN `no-store`; static `503` with numeric `Retry-After` | Exact reviewed maintenance release only | The service worker recognizes the server marker, returns the live `503` immediately, and never writes it to cache; the maintenance release's new cache version removes prior shells |
